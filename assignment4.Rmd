...
To compare lobster sizes between MPA and non-MPA sites, we used size data from 2012 and 2017. First, we tested for normality of data across all sites. For sites with normally distributed data, we tested for equal variances using Levene’s test. We then proceeded with two-sample, two-tailed t-tests. 

_Proportion of "legal" lobsters across sites_
In order to test for the proportion of legal-sized lobsters (carapace length over 82.6 mm) at all sites in 2017, we conducted a chi-square test, using lobster size data.
...
lobster_size <- ggplot(size_freq_summary, aes(x = SITE, y = mean)) +
  ...
  geom_signif(comparisons = list(c("NAPL", " CARP"), c("NAPL","CARP")), map_signif_level=TRUE, annotations = c("*", "**")) +
  labs(x = "Site", y = "Lobster Carapace Length (mm)", title = "Lobster size by site in 2017")
...
Mean lobster size (mm) in 2017 across all five sites (n(AQUE) = 67; n(CARP) = 705; n(IVEE) = 606; n(MOHK) = 178; n(NAPL) = 112) are normally distributed. From our ANOVA, we found that mean lobster size (mm) across sites differ significantly (F(`r round({size_2017_anova$parameter, 2})`) = `r round({size_2017_anova$statistic}, 2)`, p = `r round({size_2017_anova$p.value}, 2)`, $\alpha$ = 0.05).

After performing a post-hoc Tukey’s HSD test, we conclude that mean lobster size at Naples Reef differs significantly from Carpinteria (diff = 4.00 mm, *p* = 0.02), Isla Vista (diff = 4.78 mm, *p* < 0.01).  Mohawk Reef was fairly different when compared to Naples Reef, but was not statistically significant (diff = 4.23 mm, *p* = 0.06). Naples Reef also had the greatest mean lobster size (76.23 mm), suggesting that the lobster population at Naples Reef is the most robust or healthy.
...
# Side-by-side (2012 vs 2017) column graphs for mean sizes at each site
size_changes_graph <- ggplot(size_changes_summary, aes(x = SITE, y = mean)) +
  ...
  geom_errorbar(aes(ymin = mean-sd, ymax = mean+sd, width = .5, fill = as.factor(YEAR)), position = "dodge", stat = "identity") +
